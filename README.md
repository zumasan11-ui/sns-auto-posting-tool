...
Meta広告ライブラリの広告収集はApifyなしで、Mac上のブラウザ操作として実行します。Codexに「リサーチして」と指示した時だけ、`キーワードDB` の横向きジャンル表と `検索履歴DB` の実績を使って検索語を選び、`広告分析マスターDB` に1広告を直接追加します。分析中の行は黄色にし、投稿完了後に分析情報を追記して色を消します。Notion日次ページは1ページにつき1広告で作成します。
